...
func readGlobalDockerConfig() (c DockerConfig, err error) {
	...
	configFile := filepath.Join(userHome, fmt.Sprintf(".%s", OutputBuildpack), ConfigGlobal)
	_, err = os.Stat(configFile)
	if err != nil {
		if os.IsNotExist(err) {
			err = nil
...
func (c *DockerClient) buildImage(ctx context.Context, file string, tags []string, registries []DockerRegistry) (types.ImageBuildResponse, error) {
